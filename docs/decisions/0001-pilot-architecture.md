...
psychometrics service. The course-first pilot must first prove a resumable
lesson loop, prerequisite progression, and a safe content workflow. It does not
yet have the traffic, team boundaries, or asynchronous workload that justify
...
Keep course sequencing, content state transitions, learning evidence, review
scheduling, and later assessment assembly outside React components. Define
their boundaries so a worker, admin application, or psychometrics service can
be extracted later without changing the learner-facing contracts.
